...
async def me_status(
    ...
    token = await get_oauth_connection(db, current_user.id, "jira")
    if token:
        project_url = f"{token.cloud_url.rstrip('/')}/browse/{settings.jira_project_key}" if token.cloud_url else None
        return {
            "connected":   True,
            "user_name":   token.account_name,
            "user_email":  token.account_email,
    ...
    if pat:
        ...
        project_url = f"{pat.cloud_url.rstrip('/')}/browse/{project_key}" if pat.cloud_url else None
        return {
            "connected":   True,

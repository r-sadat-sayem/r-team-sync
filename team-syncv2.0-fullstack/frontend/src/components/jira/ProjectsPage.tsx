...
// ── Derive cloud base URL from the stored project_url ────────────────────────
function cloudBase(projectUrl: string | null | undefined): string | null {
  if (!projectUrl) return null;
  return projectUrl.replace(/\/browse\/.*$/, '');
}
...
function ProjectDetail({
  ...
  const openBoard = (board: JiraBoard) => {
    ...
    const path = isBusiness
      ? `jira/core/projects/${projectKey}/board`
      : `jira/software/projects/${projectKey}/boards/${board.id}`;
          ...
          <div className="flex items-center gap-2">
            {cloudUrl && (
              <Button
                variant="ghost"
                size="sm"
                onClick={() => window.open(`${cloudUrl}/browse/${projectKey}`, '_blank')}
...
export function ProjectsPage() {
  ...
  const cloud = cloudBase(jira?.project_url);
              ...
              <div className="flex items-center gap-3">
                {cloud && (
                  <button
                    ...
                    onClick={e => { e.stopPropagation(); window.open(`${cloud}/browse/${project.key}`, '_blank'); }}
                    title="Open in Jira"
                  >
                    <ExternalLink className="w-4 h-4" />
                  </button>
